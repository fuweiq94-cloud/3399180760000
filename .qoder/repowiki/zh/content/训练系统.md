...
- 训练与演示
  - train.py：Trainer 类负责训练循环控制、指标收集、可视化、模型保存与收敛检测。
...
- SnakeEnv（环境）
  - 提供 9 维局部视野观测、4 个离散动作、碰撞/超时/进食奖励与渲染。
...
- [train.py:158-215](file://train.py#L158-L215)
- [train.py:39-78](file://train.py#L39-L78)
- [train.py:93-147](file://train.py#L93-L147)
...
### SnakeEnv（环境）
- 观测空间
  - 9 维向量：8 个方向障碍物距离（负值表示障碍）、第 9 维为食物相对方向的编码。
- 动作空间
  - 离散 4 个方向：上、下、左、右。
- 奖励机制
  - 吃到食物：+10.0
...
## 结论
本训练系统以 D3QN 为核心，在贪吃蛇环境中实现了完整的强化学习训练流程。Trainer 负责训练循环控制、指标收集与可视化、模型保存与收敛检测；D3QNAgent 实现经验回放、Double DQN 更新与目标网络同步；D3QN 网络采用 Dueling 架构提升稳定性；SnakeEnv 提供清晰的观测、动作与奖励定义。系统支持 GPU 加速、内存优化与性能调优，并提供可视化与故障排除指南，便于用户快速上手与迭代改进。
...
- 观测类型（observation_type）
  - vision：局部视野 9 维；state：完整网格（维度随 grid_size 变化）。

**章节来源**
- [README.md:76-99](file://README.md#L76-L99)
- [train.py:17-28](file://train.py#L17-L28)
- [d3qn_agent.py:27-55](file://d3qn_agent.py#L27-L55)
- [snake_env.py:15-31](file://snake_env.py#L15-L31)